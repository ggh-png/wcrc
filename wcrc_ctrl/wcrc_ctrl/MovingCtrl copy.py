...
class MovingCtrl:
    def __init__(self, node: Node):
        ...
        self.target_distance = 0.95  # 예시로 설정된 목표 이동 거리
        self.target_half_distance_1 = 0.43
        self.target_half_distance_2 = 0.63
        ...
        self.control_dict = {
            ...
            'half_forward_1': self.half_go_straight_1,
            
            'half_forward_2': self.half_go_straight_2,
        ...
        self.angle_list = [-90, 0, 90, 180]
        self.angle_list2 = [180, 90, 0, -90]
    ...
    def left_turn(self):
        ...
        if self.is_step_start == False:
            ...
            # current angle - angle list = error
            min_angle = float('inf')  # ??? ? ?? ???
            min_index = 0
            for i, target_angle in enumerate(self.angle_list):
                # ?? ??? ???
                angle_diff = self.normalize_angle(self.current_theta - np.deg2rad(target_angle))
                if abs(angle_diff) < min_angle:
                    min_angle = abs(angle_diff)
                    min_index = i
            # ?? ?? ??? ??
            self.disired_theta = np.deg2rad(self.angle_list[(min_index + 1) % len(self.angle_list)])

                    

        self.logger.info("disired_theta: " + str(self.disired_theta))
    ...
    def half_go_straight_1(self):
        ...
        error = math.sqrt((self.current_pos_x - self.start_x)**2 +
                          (self.current_pos_y - self.start_y)**2) - self.target_half_distance_1
        control = Kp * error + Kd * (error - self.lastError) + Ki * (error)
        self.lastError = error
    ...
    def half_go_straight_2(self):
        ...
        error = math.sqrt((self.current_pos_x - self.start_x)**2 +
                          (self.current_pos_y - self.start_y)**2) - self.target_half_distance_2
        control = Kp * error + Kd * (error - self.lastError) + Ki * (error)
        self.lastError = error
